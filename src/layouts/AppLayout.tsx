...
export function AppLayout() {
  ...
  const visibleItems = membership
    ? menuItems.filter((item) => item.roles.includes(membership.rol))
    : []
  ...
  const currentTitle =
    pageTitles[location.pathname] ?? 'ComercioBI'
      ...
      <div className="app-main">
        <header className="topbar">
          <div className="topbar-left">
            ...
            <div>
              <h1>{currentTitle}</h1>
              <p>{company?.nombre ?? 'Empresa no disponible'}</p>
            </div>
          </div>
        ...
        <main className="content-area">
          {contextError ? (
            <section className="alert alert-error">
              <strong>No se pudo cargar todo el contexto del usuario.</strong>
